...
class Parser():
    def __init__(self):
        self.pg = ParserGenerator(
            ['NUMBER', 'PLUS', 'MINUS', 'MUL', 'DIV', 'GREATER', 'LESS',
             'EQUAL', 'ASSINGMENT', 'COMMA', 'OPEN_PAREN', 'CLOSE_PAREN',
             'OPEN_BRACKET', 'CLOSE_BRACKET', 'PRINT', 'IF', 'ELSE', 'WHILE',
             'FUNC', 'AND', 'OR', 'NOT', 'NEWLINE', 'IDENTIFIER'
             ],
    ...
    def parse(self):
        ...
        #                                     p[1]                  p[3]                                      p[7]
        @self.pg.production('statement : FUNC IDENTIFIER OPEN_PAREN def_args CLOSE_PAREN OPEN_BRACKET NEWLINE statements CLOSE_BRACKET NEWLINE')
        def statement_decfunc(p):
            return FuncDec(p[1].getstr(), p[3], p[7])

        #                               p[0]             p[2]
        @self.pg.production('def_args : IDENTIFIER COMMA def_args')
        def dec_func_multiargs(p):
            return p[2] + [p[0]]

        #                               p[0]
        @self.pg.production('def_args : IDENTIFIER')
        def dec_func_arg(p):
            return [p[0]]

        @self.pg.production('def_args : ')
        ...
        #                                            p[0]       p[1]    p[2]
        @self.pg.production('relational_expression : expression GREATER expression')
        @self.pg.production('relational_expression : expression LESS    expression')
        ...
        def error_handle(token):
            raise ValueError(
                f"{token} not expected. {token.getsourcepos()} - {token.gettokentype()}")
